    ...
    try {
        ...
        $db->execute(
            "INSERT INTO exchanges
             ...
             VALUES (?, ?, ?, ?, CURDATE(), 'different_product', ?, ?, ?, 'completed', ?, ?)",
            [
                $exchangeNumber,
                $invoiceId,
                $customerId,
        ...
        // Process exchange: inventory switching and balance adjustment
        require_once __DIR__ . '/../includes/return_processor.php';
        
        // Prepare items for inventory movement
        $returnItemsForInventory = [];
        foreach ($processedReturnItems as $item) {
            $returnItemsForInventory[] = [
                'product_id' => $item['product_id'],
                'quantity' => $item['quantity'],
                'unit_price' => $item['unit_price'],
                'batch_number' => $item['batch_numbers'] ?? null,
                'batch_number_id' => !empty($item['batch_number_ids']) ? $item['batch_number_ids'][0] : null,
            ];
        }
        
        $replacementItemsForInventory = [];
        foreach ($processedReplacementItems as $item) {
            $replacementItemsForInventory[] = [
                'product_id' => $item['product_id'],
                'quantity' => $item['quantity'],
                ...
                'batch_number_id' => $item['batch_number_id'] ?? null,
            ];
        }
        
        // Process inventory switching
        $inventoryResult = processExchangeInventory(
            $returnItemsForInventory,
            $replacementItemsForInventory,
            $customerId,
            $salesRepId,
        ...
        // Process customer balance adjustment
        $customerBalance = (float)($customer['balance'] ?? 0);
        
        if (abs($difference) < 0.01) {
            // No balance change
            $balanceNote = 'لا يوجد فرق مالي';
        } elseif ($difference < 0) {
            // New product is cheaper - customer should receive value
            $absDifference = abs($difference);
            if ($customerBalance > 0) {
                // Reduce debt
                $newBalance = max(0, round($customerBalance - $absDifference, 2));
            } else {
                // Add to credit
                $newBalance = round($customerBalance - $absDifference, 2);
            }
            $db->execute("UPDATE customers SET balance = ? WHERE id = ?", [$newBalance, $customerId]);
            $balanceNote = "تم إضافة {$absDifference} ج.م لرصيد العميل الدائن";
        } else {
            // New product is more expensive - customer should pay difference
            $newBalance = round($customerBalance + $difference, 2);
            $db->execute("UPDATE customers SET balance = ? WHERE id = ?", [$newBalance, $customerId]);
            $balanceNote = "تم إضافة {$difference} ج.م لدين العميل";
        }
        ...
        returnJson([
            'success' => true,
            'message' => 'تم إنشاء الاستبدال بنجاح',
